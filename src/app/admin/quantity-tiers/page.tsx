...
export default function QuantityTiersPage() {
    ...
    const [dialogState, setDialogState] = useState<{open: boolean; tier?: Partial<QuantityTier>}>({ open: false, tier: undefined });
    ...
    const { data: tiers, isLoading: isLoadingData, error } = useCollection<QuantityTier>(tiersQuery);
    const isLoading = authLoading || isLoadingData;
    
    useEffect(() => {
        if (dialogState.open) {
            setQuantity(dialogState.tier?.quantity || 0);
            setPricePerUnit(dialogState.tier?.pricePerUnit || 0);
            setSave(dialogState.tier?.save || 0);
        }
    }, [dialogState.open, dialogState.tier]);
    ...
    const handleSaveTier = async () => {
        ...
        try {
            if (dialogState.tier?.id) {
                await updateDoc(doc(db, 'quantityTiers', dialogState.tier.id), data);
                toast({ title: 'Success', description: 'Tier updated.' });
            } else {
                await addDoc(collection(db, 'quantityTiers'), {
                    ...data,
                    createdAt: serverTimestamp(),
                });
                toast({ title: 'Success', description: 'New tier added.' });
            }
            setDialogState({ open: false, tier: undefined });
        } catch (e: any) {
            console.error(e);
    ...
    const handleDeleteTier = async (id: string) => {
        ...
        try {
            await deleteDoc(doc(db, 'quantityTiers', id));
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="font-headline text-3xl font-bold">Quantity Tiers</h1>
                    <p className="text-muted-foreground">Manage quantity-based pricing for products.</p>
                </div>
                <Button onClick={() => setDialogState({ open: true, tier: {} })}>
                    <PlusCircle className="mr-2 h-4 w-4" />Add Tier
                </Button>
            </div>
            <Card>
                <CardHeader>
                    <CardTitle>All Quantity Tiers</CardTitle>
                        ...
                        <TableBody>
                            ...
                            {!isLoading && tiers?.map((tier) => (
                                <TableRow key={tier.id}>
                                    ...
                                    <TableCell>${tier.pricePerUnit.toFixed(3)}</TableCell>
                                    <TableCell>{tier.save}%</TableCell>
                                    <TableCell>{tier.createdAt ? format(tier.createdAt.toDate(), 'MMM d, yyyy') : 'N/A'}</TableCell>
                                    <TableCell className="text-right">
                                         <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                <DropdownMenuItem onSelect={() => setDialogState({ open: true, tier })}>
                                                    <Edit className="mr-2 h-4 w-4" />
                                                    <span>Edit</span>
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => handleDeleteTier(tier.id)} className="text-red-600 focus:text-red-600 focus:bg-red-50">
            ...
            <Dialog open={dialogState.open} onOpenChange={handleOpenChange}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{dialogState.tier?.id ? 'Edit Tier' : 'Add New Tier'}</DialogTitle>
