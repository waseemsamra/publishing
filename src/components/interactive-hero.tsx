...
export function InteractiveHero() {
  ...
  const [initialFeatured, setInitialFeatured] = useState<Category | null>(null);
  ...
  useEffect(() => {
    if (allCategories) {
      ...
      setInitialFeatured(initial);
      setFeaturedItem(initial);

      if (initial) {
        setGridItems(
          topLevelCategories.filter((cat) => cat.id !== initial.id).slice(0, 12)
        );
      } else {
  ...
  if (!initialFeatured && gridItems.length === 0) {
    return (
